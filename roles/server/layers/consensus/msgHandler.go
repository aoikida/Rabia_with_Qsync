...
func (c *Consensus) binConMsgHandling(msg Msg) {
	seq := msg.Obj.SvrSeq
	if ok := c.UpdateTermIfNecessary(seq, false); !ok && msg.Type != PQcheck{
		c.OlderThanTermMsg++
		return
	}
	...
	Phase := msg.Phase
	Value := msg.Value
	if c.Ledger[slot].IsDone && msg.Type != PQcheck{
		return
	}
	switch msg.Type {
	case Proposal:
	...
	case PQcheck:
		...
		if !ProxySeqIdEqual(msg.Obj, &c.Ledger[slot].MyProposal) {
			c.SyncQlock.Lock()
